    ...
    <div className="page debate-page">
      ...
      <div className="debate-layout">
        ...
        <aside className={`debate-sidebar ${showDefinitions ? "open" : ""}`}>
          <div className="sidebar-close-header">
            <div>
              <span style={{ display: "block", fontSize: "var(--font-size-base)", fontWeight: 700 }}>📋 Panel</span>
              <Link href="/dashboard" style={{ fontSize: "var(--font-size-xs)", color: "var(--accent-primary)", textDecoration: "none" }}>
                ← Volver al Dashboard
              </Link>
            </div>
            <button onClick={() => setShowDefinitions(false)} className="sidebar-close-btn">✕ Cerrar</button>
          </div>
          <div className="debate-sidebar-content">
          ...
          <div className="sidebar-footer-mobile">
            <button onClick={() => setShowDefinitions(false)} className="btn btn-primary btn-full btn-lg">
              Cerrar Panel
            </button>
          </div>
        </aside>
        ...
        <main className="debate-chat">
          <div className="mobile-chat-header">
            <div className="header-info">
              <span className="header-title">{debate.title}</span>
              <span className="header-status">
                {debate.status === 'active' ? '🟢 En vivo' :
                  debate.status === 'paused' ? '⏸️ Pausado' :
                    debate.status === 'setup' ? '⏳ Esperando' : '🏁 Finalizado'}
              </span>
            </div>
            <button onClick={() => setShowDefinitions(true)} className="btn btn-ghost btn-sm info-btn">
              📋 Info
            </button>
          </div>
        ...
        {/* Toggle flotante eliminado - ahora está en el Header */}
      </div>
      ...
      <style jsx>{`
        ...
        .debate-layout {
          ...
          overflow: hidden;
          position: relative;
          /* z-index: 1; REMOVED to allow sidebar to cover navbar */
        }

        /* Sidebar — Mobile-first (base = fullscreen overlay) */
        .debate-sidebar {
          position: fixed;
          top: 0;
          left: 0;
        ...
        @media (min-width: 769px) {
          ...
          .debate-layout {
            grid-template-columns: 300px 1fr;
            z-index: 1; /* Restore z-index on desktop if needed */
          }
          ...
          /* Hide mobile header link on desktop */
          .sidebar-close-header {
             display: none;
          }
        }
